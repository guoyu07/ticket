...
//                	URL url = new URL(userInfoBean.getAvatar().getAvatarURL30());//你要登陆网站的登陆界面的地址. 
//                	HttpURLConnection con = (HttpURLConnection) url.openConnection(); 
//                	con.setDoOutput(true); // POST方式 
//
//                	con.setRequestMethod("POST"); 
//                	InputStream is = con.getInputStream(); // 输出流，写数据 
//                	int count = is.available();
//                	byte[] b = new byte[count];
//                	is.read(b, 0, count);
//                	String imgpath = ServletActionContext.getRequest().getRealPath("/upload") + "/" + new Date().getTime();
...
//                	os.close();
					...
					try {
						memberQQ = memberQqService.add(openID, userInfoBean.getNickname(), userInfoBean.getGender(), null);
					} catch (ServiceException e) {
						e.printStackTrace();
					}
                	
                	//转发
                	ActionContext.getContext().getSession().put(ContextConstants.SCOPE_SYSTEM_USER, memberQQ);
                	request.setAttribute(ContextConstants.SCOPE_SYSTEM_USER, memberQQ);
                	try {
