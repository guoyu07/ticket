								...
								<table class="my_table100">
								  ...
								  <s:if test="pageModule != null && pageModule.totalCount > 0">
								  	<s:iterator id="c" value="pageModule.pageList" status="st">
								  		<tr>
								  			<td><input  name="${actionName }CheckBox" type="checkbox" value="${cId}"></td>
										    <td>${cId}</td>
										    ...
										    <td>属性值</td>
										    <td><span>
										    	<a href="#" class="logicDeleteBtn" entityName="${entityName }" value="${cId}">删除</a>
										    	<a href="/${actionName }_edit.action?id=${cId}&versionFlag=${versionFlag }">编辑</a></span></td>
										 </tr>
								  	</s:iterator>
								  </s:if>
								  <s:else>
