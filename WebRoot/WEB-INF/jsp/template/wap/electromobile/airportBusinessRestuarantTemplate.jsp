					...
					<div class="mr30 line-big tit_tabs">
						...
						<s:if test="#request.businessTopTypeList != null">
							<s:iterator id="businessTopType" value="#request.businessTopTypeList" status="st">
								<s:if test="#st.first">
									<div class="x4">
										<a href="#" class="button bg-yello d_button block text-center selected businessTopType" typeId="${businessTopType.id }">
										<i class="icon-caret-down"></i>${businessTopType.name }</a>
									</div>
								</s:if>
								<s:else>
									<div class="x4">
										<a href="#" class="button bg-sub d_button block text-center businessTopType" typeId="${businessTopType.id }">
										<i class="icon-caret-down"></i>${businessTopType.name }</a>
									</div>
								</s:else>
									...
									<s:if test="#st.first">
										...
										<s:if test="#request.businessSecondTypeList != null">
											<s:iterator id="businessSecondType" value="#request.businessSecondTypeList" status="st">
												...
												<s:else>
													<a href="#" class='x3 fz20 businessInfoListByType' typeId="${businessSecondType.id }">${businessSecondType.name }<span class='float-right'>|</span></a>
												</s:else>
											</s:iterator>
										</s:if>
									</s:if>
														...
														<div class="media media-x">
															...
															<div class="media-body line businessDetail" businessId="${businessInfo.id }">
																...
																<div class="x6 fz22 padding-top">
																	人均
																	<font class="c_orange fz22">￥${businessInfo.averagePrice }</font>
																</div>
