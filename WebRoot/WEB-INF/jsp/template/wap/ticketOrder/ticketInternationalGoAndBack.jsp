				...
				<div class="mobile-main">
					<div class="order_ls">
						<div class="tit_tab line mr20">
							...
							<div class="x4" style="width:25%;">
								<a href="/ticketOrder_internationalGoAndBack.action?direct=true" class="selected"><i class="icon-caret-down"></i>国际往返</a>
							</div>
						</div>
					</div>
							...
							<li style="padding: 20px;">
								<div class="fly_line clearfix">
									<s:if test="#session.internationalDepartureName != null">
										<span id="city1" class='float-left selectCityClass' ticketType="international" buyType="goAndBack" cityType="departure" cityCode="${internationalDepartureCode }"><ticket:format value="${internationalDepartureName }" size="3" /><em>出发城市</em></span>
									</s:if>
									<s:else>
										<span id="city1" class='float-left selectCityClass' ticketType="international" buyType="goAndBack" cityType="departure" cityCode="KMG">昆明<em>出发城市</em></span>
									</s:else>
									<s:if test="#session.internationalArrivalName != null">
										<span id="city2" class='float-right selectCityClass' ticketType="international" buyType="goAndBack" cityType="arrival" cityCode="${internationalArrivalCode }"><ticket:format value="${internationalArrivalName }" size="3" /><em>到达城市</em></span>
									</s:if>
									<s:else>
										<span id="city2" class='float-right selectCityClass' ticketType="international" buyType="goAndBack" cityType="arrival" cityCode="HKG">香港<em>到达城市</em></span>
									</s:else>
								</div>
							</li>
						</ul>
