				...
				<div id="viewbox" class="site_right_box">
					<table class="my_table100 margin_top10">
					    ...
					    <tr  class="text_align_left">
					      <td><span>
					      	<a href="#" class="batchProcessBtn" value="${actionName}CheckBox" entityName="${entityName}">批量回复</a>
					      </span></td>
					    </tr>
					    <tr>
					      <td>
								<table class="my_table100">
								  <tr class="my_table_top1">
								    ...
								    <td width="80" class="text_align_center">内容</td>
								    <td width="80" class="text_align_center">涉及制度</td>
								    <td width="80" class="text_align_center">运质意见</td>
								    <td width="80" class="text_align_center">运质备注</td>
								    <td width="80" class="text_align_center">处理结果</td>
								    <td width="80" class="text_align_center">备注</td>
								    <td width="80" class="text_align_center">状态</td>
								    <td width="80" class="text_align_center">问题是否关闭</td>
								  ...
								  <s:if test="pageModule != null && pageModule.totalCount > 0">
									<s:iterator var="c" value="pageModule.pageList" status="st">
								  		<tr>
									    	...
									    	<td>${c.evaluation.involveSystem }</td>
									    	<td>${c.evaluation.shipmentQualityOpinion }</td>
									    	<td>${c.evaluation.remark }</td>
									    	<td>${c.msg }</td>
									    	<td>${c.remark }</td>
									    	<td>${c.state.value }</td>
									    	<td>${c.close ? '是' : '否'}</td>
										    <td>
										    	<s:if test="#c.close">
										    	<a href="#" class="processBtn" entityName="${entityName}" value="${c.id }">查看</a>
										    	</s:if>
										    	<s:else>
										    	<a href="#" class="feedbackBtn" entityName="${entityName}" value="${c.id }">回复</a>
										    	</s:else>
										    	<s:if test="!#c.top">
										    	<a href="#" class="topBtn" entityName="${entityName}" value="${c.id }">置顶</a>
										    	</s:if>
										    	<s:else>
										    	<a href="#" class="topBtn" entityName="${entityName}" value="${c.id }">取消置顶</a>
										    	</s:else>
										    </td>
										</tr>
								  	</s:iterator>
