								  	...
								  	<s:iterator id="c" value="pageModule.pageList" status="st">
								  		<tr>
										    ...
										    <td>
										     ...
										     <td><ticket:systemCommon type="channelCustomerInfoObj" value="${c.channelCustomerInfo_id}"/>
									    	 ${channelCustomerInfoObj.name }
									    	 </td>
										    
										    <td><s:date name="#c.applyDate" format="yyyy-MM-dd"/></td>
										    <td><s:date name="#c.auditDate" format="yyyy-MM-dd"/></td>
										    <td><s:date name="#c.signingDate" format="yyyy-MM-dd"/></td>
										    <td>${c.firstRenewMoney }</td>
										    <td><span>
										    	<s:if test="#c.agreementState == 3">
										    		<a href="/${actionName}_fafang.action?id=${c.id}">二次审核</a>
										    		<a href="/${actionName}_qh.action?id=${c.id }&versionFlag=${versionFlag}">编辑</a>
										    	</s:if>
										    	<s:if test="#c.agreementState == 4">
										    		<a href="javascript:;" class="guidangAgreement" 
										    				attrValue="${c.id}">归档</a>
										    	</s:if>
										    	<s:if test="#c.agreementState == 5">
										    		合同已归档
										    	</s:if>
										    	...
										    	<s:if test="#c.effective == 1 && #c.agreementState != 5">
